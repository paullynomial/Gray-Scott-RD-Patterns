...
class DifferenceUniformGrid(Difference):
    ...
    def _make_stencil(self, grid):
        self.center_stencil = self._coeffs(grid, self.j)

    def _build_matrix(self, grid):
        shape = [grid.N] * 2
        matrix = sparse.diags(self.center_stencil, self.j, shape=shape)
        matrix = matrix.tolil()
        jmin = -self.j.min()
        if jmin > 0:
            for i in range(jmin):
                if isinstance(grid, UniformNonPeriodicGrid):
                    ...
                    matrix[i, -jmin + i:] = self.center_stencil[:jmin - i]
        ...
        if jmax > 0:
            for i in range(jmax):
                if isinstance(grid, UniformNonPeriodicGrid):
                    ...
                    matrix[-jmax + i, :i + 1] = self.center_stencil[-i - 1:]
